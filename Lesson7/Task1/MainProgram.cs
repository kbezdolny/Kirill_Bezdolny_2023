            ...
            if (command != null)
            {
                ...
                switch (parameters["type"])
                {
                    ...
                    case "-o":
                        ...
                        {
                            var fileContent = WorkingWithFiles.GetLinesAsync(elementName).GetAwaiter().GetResult();
                            if (fileContent != null)
                    ...
                    case "-c":
                        ...
                        {
                            WorkingWithFiles.CopyAsync(allPaths?[0], allPaths?[1]).GetAwaiter().GetResult();
                        }
                        else if (Directory.Exists(WorkingWithDirectory.GetNewPath(allPaths?[0])))
                        {
                            WorkingWithDirectory.CopyAsync(allPaths?[0], allPaths?[1]).GetAwaiter().GetResult();
                        }
                        break;
                    case "-m":
                        if (File.Exists(WorkingWithDirectory.GetNewPath(allPaths?[0])))
