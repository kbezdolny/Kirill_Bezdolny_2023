...
public static class WorkingWithDirectory
{
    ...
    public static async Task CopyAsync(string? dirName, string? toPath = null)
    {
        ...
        foreach (FileInfo file in files)
        {
            ...
            await using FileStream destinationStream = File.Create(Path.Combine(toPath, file.Name));

            await sourceStream.CopyToAsync(destinationStream);
        }
        foreach (DirectoryInfo subdir in dirs)
        {
            await CopyAsync(Path.Combine(path, subdir.Name), toPath);
        }
    }
