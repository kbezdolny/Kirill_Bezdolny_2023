...
Console.WriteLine("\n>>>Selected customers:");
var selectedCustomers = from c in customersList where c.name == name select c;
foreach (var customer in selectedCustomers)
{
    Console.WriteLine($"> {customer.name} - {customer.age} years - {customer.address}");
}

Console.WriteLine($"> Quantity customers over 18 years old: {(from c in selectedCustomers where c.age > 18 select c).ToList().Count}");
